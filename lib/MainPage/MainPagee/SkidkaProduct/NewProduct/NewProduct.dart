...
import 'package:serpay/MainPage/MainPagee/SkidkaProduct/NewProduct/NewProductGrid.dart';
import 'package:serpay/MainPage/MainPagee/SkidkaProduct/NewProduct/NewProductList.dart';
...
class _NewProductModelState extends State<NewProductModel> {
  ...
  Future<bool> fetchAlbum({bool isrefest = false}) async {
    ...
    final response = await http.get(Uri.parse(
        ...
        widget.sort == 0
        ? "$ip/users/products/new?offset=$current_page"
        : widget.sort == 1
            ? "$ip/users/products/new?offset=$current_page&sort=1"
            : "$ip/users/products/new?offset=$current_page&sort=0"
    : widget.sort == 0
            ? "$ip/public/products/new?offset=$current_page"
            : widget.sort == 1
            ? "$ip/public/products/new?offset=$current_page&sort=1"
            : "$ip/public/products/new?offset=$current_page&sort=0" ),
    ...
    if (response.statusCode == 200) {
      ...
      newProducy = newProductFromJson(response.body);
        ...
        body: SmartRefresher(
            ...
            child: CustomScrollView(
              slivers: [
                ...
                widget.checkpage == true
                    ? NewProductList(newProduct: prod)
                    :NewProductGrid(newProduct: prod)
              ],
            )));
  }
}
