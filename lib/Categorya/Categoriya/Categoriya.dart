...
class _CateProductState extends State<CateProduct> {
  ...
  Future<bool> fetchAlbum({bool isrefest = false}) async {
    ...
    final response = await http.get(Uri.parse(
        ...
        widget.sort == 0
        ? "$ip/users/products/categories/${widget.subcatId}?offset=$current_page"
        : widget.sort == 1
        ...
        : widget.sort == 0
            ? "$ip/public/categories/products/${widget.subcatId}?offset=$current_page"
            : widget.sort == 1
            ? "$ip/public/categories/products/${widget.subcatId}?offset=$current_page&sort=1"
            : "$ip/public/categories/products/${widget.subcatId}?offset=$current_page&sort=0"),
